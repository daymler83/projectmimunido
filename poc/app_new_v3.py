...
@app.route('/')
def home():
    return render_template('index_indi.html', categories=categories, is_home=True)

# Route to render Page 2 (back from Page 3)
@app.route('/indicator_list', methods=['GET'])
def indicators_list():
    return render_template('index_indi.html', categories=categories, is_home=False)  # Replace with your Page 2 template
